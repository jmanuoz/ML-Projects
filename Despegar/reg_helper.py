...
def fit_and_get_regions(X_train, y_train, X_test, y_test, degree = 2, lambd = 0, plot_it = True, print_it = False):
    ...
    # Defino el modelo de clasificacion como Regresion Logistica
    if lambd == 0:
        C1 = 10000000000
    else:
        C1 = 1/lambd 
    ...
    if plot_it:
        ...
        print('Regresion Logistica Polinomial de orden '+str(degree) +', con lamdba (regularizacion L2):' +  str(lambd))
        plt.show()
    if print_it:
        print('Train Accuracy (Exactitud):',score_train_logist_pol)
        print('Test Accuracy (Exactitud):',score_test_logist_pol)
...
def get_two_layer_model_L2(input_shape, output_size, hidden_units=10, lr=0.1, l2_lambda=0, decay=0.0, initializer='normal', l1_lambda=0, optim = None):
    ...
    return model
def get_three_layer_model_L2(input_shape, output_size, hidden_units=10,hidden_units2=10, lr=0.1, l2_lambda=0, decay=0.0, initializer='normal', l1_lambda=0, optim = None):
    model = Sequential()
    if optim is None:
        optim = optimizers.adam(lr=lr, decay=decay)
    regularizer = regularizers.l2(0)
    ...
    model.add(Dense(hidden_units2,input_dim=hidden_units,  
                    kernel_initializer=initializer, 
                    bias_initializer=initializer,
                    activation='sigmoid', 
                    kernel_regularizer=regularizer, 
